...
const mutations = {
  ...
  setUserDepartment: (state, userDepartment) => (state.userDepartment = userDepartment)
};
...
const actions = {
  ...
  async getUserDepartment({commit, getters}){
    try {
      const response = await axios.get(`${baseUrl}/staff/user-department`,{headers: {Authorization: `jwt ${getters.authToken}`} })
